...
module.exports = { 
    config: {
        name: "banner",
        description: "Set the banner of your clan.",
        usage: "<link>",
        category: "general",
        accessableby: "Member",
        aliases: []
    ...
    run: async (client, message, args) => {
        ...
        const embed = new MessageEmbed()
            .setColor("#00FF00")
            .setTitle("Clan Banner")
            .setDescription(`\`${message.author.tag}\` has changed the banner of their clan`)
            .setThumbnail(clan.clan_icon)
            .setFooter({ text: `Clan Tag: ${clan.clan_tag}`});

        message.channel.send({ embeds: [embed] });

        clan.clan_banner = args.join(" ");
        clan.save().then(() => {
            const embed = new MessageEmbed()
                .setColor("#00FF00")
                .setTitle("Clan Banner")
