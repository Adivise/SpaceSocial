...
module.exports = { 
    ...
    run: async (interaction, client) => {
        ...
        if (user.money < config.gacha.gacha_price) return interaction.editReply(`You need ${numberWithCommas(config.gacha.gacha_price)} coins to open gacha.`);
        ...
        if (ticket.guarantee_leg >= config.gacha.guarantee_1) {
            /// Legendary
            item = tickets[4];
            ticket.guarantee_leg = 0;
            ticket.guarantee_myth += 1;
            ticket.legendary_ticket += 1;
        } else if (ticket.guarantee_myth >= config.gacha.guarantee_2) {
            /// Mythical
            item = tickets[5];
        ...
        const embed = new EmbedBuilder()
            ...
            .setFooter({ text: `Guarantee (Legendary ${config.gacha.guarantee_1 - ticket.guarantee_leg}) & (Mythical ${config.gacha.guarantee_2 - ticket.guarantee_myth}) Rounds` });

        interaction.editReply({ embeds: [embed] });

        ticket.gacha_cooldown = Date.now() + (ticket.gacha_cooldown_time * 1000);
        user.money -= config.gacha.price;
